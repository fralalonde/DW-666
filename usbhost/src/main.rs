...
use atsamd_usb_host::{SAMDHost, Pins, HostEvent};
...
fn main() -> ! {
    ...
    runtime::spawn(async {
        loop {
            unsafe { USB_HOST.assume_init_mut().update(NoEvent, USB_DRIVERS.as_mut()).await }
...
unsafe fn USB() {
    // debug!("big");
    let host_event = USB_HOST.assume_init_ref().get_event();
    runtime::spawn(
        async move {
            unsafe { USB_HOST.assume_init_mut().update(host_event, USB_DRIVERS.as_mut()).await }
        })
}
