...
mod app {
    ...
    const BEATSTEP: Interface = Interface::Serial(0);
    ...
    fn usb_interrupt(mut cx: usb_interrupt::Context) {
        ...
        cx.shared.usb_midi.lock(|u| if u.poll() {
            while let Some(packet) = u.receive().unwrap() {
                if let Err(e) = midi_route::spawn(Src(BEATSTEP), PacketList::single(packet)) {
            ...
            loop {
                match bstep.receive() {
                    Ok(Some(packet)) => {
                        ...
                        midi_route::spawn(Src(BEATSTEP), PacketList::single(packet)).unwrap();
                        continue;
                    }
                    Err(e) => {
    ...
    fn usart2_irq(mut cx: usart2_irq::Context) {
        cx.shared.dw6000.lock(|dw6000| {
            ...
            while let Ok(Some(packet)) = dw6000.receive() {
                midi_route::spawn(Src(DW6000), PacketList::single(packet)).unwrap();
            }
        });
    }
    ...
    fn midi_route(mut cx: midi_route::Context, binding: Binding, packets: PacketList) {
        cx.shared.midi_router.lock(|r| r.midi_route(packets, binding).unwrap());
    }

    // TODO split output queues (one task per interface)
    #[task(shared = [usb_midi, dw6000, beatstep], capacity = 128, priority = 2)]
    fn midi_send(mut cx: midi_send::Context, interface: Interface, packets: PacketList) {
        match interface {
            // includes BeatStep
            Interface::USB(_) => cx.shared.usb_midi.lock(
                |midi| if let Err(e) = midi.send(packets) {
                    rprintln!("Failed to send USB MIDI: {:?}", e)
                }),

            DW6000 => cx.shared.dw6000.lock(
                |dw6000| if let Err(e) = dw6000.send(packets) {
                    rprintln!("Failed to send Serial MIDI: {:?}", e)
                }),

            BEATSTEP => cx.shared.beatstep.lock(
                |midi| if let Err(e) = midi.send(packets) {
                    rprintln!("Failed to send Serial MIDI: {:?}", e)
                }),
            _ => {}
