...
impl UsbMidi {
    ...
    fn send(&mut self, frament: MidiFragment) {
        if self.usb_dev.state() == UsbDeviceState::Configured {
            self.midi_class.send(fragment);
        }
    }
}
...
impl<B: UsbBus> MidiClass<'_, B> {
    ...
    pub fn send(&mut self, fragment: MidiFragment) -> Result<usize> {
        self.standard_bulkin.write(fragment.as_bytes())
    }
    ...
    fn get_configuration_descriptors(&self, writer: &mut DescriptorWriter) -> Result<()> {
        ...
        writer.write(
            ...
            &[
                MS_HEADER_SUBTYPE,
                0x00,0x01, // Revision
                // Total size of class specific descriptors? (little endian?)
                (0x07 + MIDI_OUT_SIZE), 0x00
            ]
        )?;
